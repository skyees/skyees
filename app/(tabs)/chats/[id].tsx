...
const ChatPage = () => {
 ...
 const messageHandler = (newMessage: any) => {
  ...
  // 🛑 ADD THIS LINE:
  // If the message came from ME, don't add it again (onSend already did it)
  if (newMessage.senderId === user?.id) {
    console.log("⏭️ Ignoring my own message echo");
    return;
  }

  setImessages((prev) => {
    // Standard duplicate check by ID
    ...
    return GiftedChat.append(prev, [
      {
        ...
        image: newMessage.image, // Fix image duplication too
  ...
  const renderChatFooter = () => {
    ...
    // Only show reply bar if not editing
    if (replyMessage) {
        ...
        <GiftedChat
                  ...
                  renderCustomView={(props) =>
                    props.currentMessage.edited ? (
                      <Text style={{ fontSize: 10, color: 'gray', marginTop: 2, alignSelf: 'flex-end' }}>
                        (edited)
                      </Text>
                    ) : null
                  }
